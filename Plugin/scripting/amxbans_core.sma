...
public plugin_init()
{
	...
	register_cvar("amx_vote_ratio", "0.02", FCVAR_PROTECTED)
	register_cvar("amx_vote_time", "10", FCVAR_PROTECTED)
	register_cvar("amx_vote_answers", "1", FCVAR_PROTECTED)
	register_cvar("amx_vote_delay", "60", FCVAR_PROTECTED)
	register_cvar("amx_last_voting", "0", FCVAR_PROTECTED)
	register_cvar("amx_show_activity", "2", FCVAR_PROTECTED)
	register_cvar("amx_votekick_ratio", "0.40", FCVAR_PROTECTED)
	register_cvar("amx_voteban_ratio", "0.40", FCVAR_PROTECTED)
	register_cvar("amx_votemap_ratio", "0.40", FCVAR_PROTECTED)
...
// amxbans
	pcvarip = register_cvar("amxbans_server_address", "", FCVAR_PROTECTED)
	pcvarprefix = register_cvar("amx_sql_prefix", "amx", FCVAR_PROTECTED)
	pcvaradminsfile = register_cvar("amxbans_use_admins_file", "0", FCVAR_PROTECTED)
...
//amxbans
	...
	formatex(pquery, charsmax(pquery),"SELECT aa.steamid,aa.password,aa.access,aa.flags,aa.nickname,ads.custom_flags,ads.use_static_bantime \
...
//
	if(SQL_NumRows(query))
	{
		...
		while (SQL_MoreResults(query))
		{
			...
			SQL_ReadResult(query, qcolStatic, Static, charsmax(Static));
			SQL_ReadResult(query, qcolCustom, Access, charsmax(Access));
			...
			trim(Access)
			if(equal(Access,"")) SQL_ReadResult(query, qcolAccess, Access, charsmax(Access));
...
accessUser(id, name[] = "")
{
	...
	new userip[16], userauthid[32], password[32], passfield[32], username[32]
	...
	if (result & 8)
	{
		...
		if( !(get_user_flags(id) & ADMIN_USER) )
			engclient_print(id, engprint_console, "* Type ^"admin_help^" in console to retrieve a list of available admin commands")
	}
