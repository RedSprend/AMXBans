...
public cmdBanDisconnectedMenu(id,level,cid)
{
	...
	if(!dnum)
	{
		client_print(id,print_chat,"%L",id,"NO_DISCONNECTED_PLAYER_IN_LIST")
...
public cmdMenuBanDisc(id)
{
	...
	new data[3]
	data[0] = id
	SQL_ThreadQuery(g_SqlX, "_cmdMenuBanDisc", pquery, data, sizeof(data))
...
public _cmdMenuBanDisc(failstate, Handle:query, error[], errnum, data[], size)
{
	...
	formatex(pquery, charsmax(pquery), "INSERT INTO `%s%s` (player_id,player_ip,player_nick,admin_ip,admin_id,admin_nick,ban_type,ban_reason,ban_created,ban_length,server_name,server_ip,expired) \
			...
			g_dbPrefix, tbl_bans, g_choicePlayerAuthid[id], g_choicePlayerIp[id], player_name, admin_ip, admin_authid, admin_nick, g_ban_type[id], g_choiceReason[id], g_choiceTime[id], servername_safe, g_ip, g_port)

	new data[3]
	data[0] = id
	SQL_ThreadQuery(g_SqlX, "__cmdMenuBanDisc", pquery, data, sizeof(data))
...
public __cmdMenuBanDisc(failstate, Handle:query, error[], errnum, data[], size)
{
	...
	{
		client_print(id,print_console,"%L",id,"BAN_DISCONNECTED_PLAYER_SUCCESS")
	}
	else
	{
		client_print(id,print_console,"%L",id,"BAN_DISCONNECTED_PLAYER_FAILED")
	}
...
{
	if(get_pcvar_num(pcvar_debug) < 4)
		return PLUGIN_CONTINUE
