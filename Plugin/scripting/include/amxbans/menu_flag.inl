...
public actionFlagReasonMenu(id,menu,item)
{
	...
	if(aid == 99)
	{
		...
		client_cmd(id,"messagemode custom_ban_reason")
		menu_destroy(menu)
		return PLUGIN_HANDLED
	}
	else
...
FlagPlayer(id)
{
	...
	new anick[64], aauthid[32], aip[16], pname[64]
	...
	SQL_ThreadQuery(g_SqlX, "_FlagPlayer", pquery, data, sizeof(data))
...
UnflagPlayer(id,announce=0)
{
	...
	SQL_ThreadQuery(g_SqlX, "_UnflagPlayer", pquery, data, sizeof(data))
...
public _FlagPlayer(failstate, Handle:query, error[], errnum, data[], size)
{
	...
	if(failstate)
	{
		client_print(id,print_chat,"%L",data[0],"FLAGG_MESS_ERROR",g_choicePlayerName[id])
		new szQuery[256]
	...
	{
		client_print(id,print_chat,"%L",data[0],"FLAGG_MESS",g_choicePlayerName[id])
...
public _UnflagPlayer(failstate, Handle:query, error[], errnum, data[], size)
{
	...
	if(failstate)
	{
		client_print(id,print_chat,"%L",data[0],"UN_FLAGG_MESS_ERROR",g_choicePlayerName[id])
	...
	if(SQL_AffectedRows(query))
	{
		if(data[1])
		{
			client_print(id,print_chat,"%L",id,"UN_FLAGG_MESS",g_choicePlayerName[id])
