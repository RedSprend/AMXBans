...
public banmod_online(id)
{
	...
	strtok(ip_port, g_ip, charsmax(g_ip), g_port, charsmax(g_port), ':')
...
public banmod_online_(failstate, Handle:query, error[], errnum, data[], size)
{
	...
	else
	{
		...
		SQL_ReadResult(query, 0, kick_delay_str, charsmax(kick_delay_str))
	...
	SQL_ThreadQuery(g_SqlX, "banmod_online_update", pquery, szData, sizeof(szData))
...
public fetchReasons(id)
{
	...
	if(g_SqlX)
	{
		SQL_ThreadQuery(g_SqlX, "fetchReasons_", pquery, data, sizeof(data))
	}
	else
	{
