...
*/
	...
	if(isset($_POST["del"]) && $_SESSION["loggedin"]) {
		...
		log_to_db("Admin config","Deleted admin: ".sql_safe($_POST["username"]));
	}
	...
	if(isset($_POST["save"]) && $_SESSION["loggedin"]) {
		...
		if(!$user_msg) {
			...
			log_to_db("Admin config","Edited admin: ".sql_safe($_POST["username"])." (nick: ".sql_safe($_POST["nickname"]).")");
		}
	}
	//amxadmin add
	if(isset($_POST["new"]) && $_SESSION["loggedin"]) {
		...
		if(!$user_msg) {
			...
			log_to_db("Admin config","Added admin: ".$name);
		} else {
			$input=array(
	...
	$smarty->assign("yesno_choose",array("no","yes"));
	$smarty->assign("yesno_output",array("_NO","_YES"));
	$smarty->assign("ashow_output",array("_YES","_NO"));
	$smarty->assign("ashow",array(1,0));
